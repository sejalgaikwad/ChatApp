...
// Create program
program.version(version).description("Installs the specified package along with correct peerDeps.").option("-d, --dev", "Install the package as a devDependency").option("-g, --global", "Install the package globally").option("-o, --only-peers", "Install only peerDependencies of the package").option("-S, --silent", "If using npm, don't save in package.json").option("-Y, --yarn", "Install with Yarn").option("-r, --registry <uri>", "Install from custom registry (defaults to NPM registry)").option("--dry-run", "Do not install packages, but show the install command that will be run").option("-a, --auth <token>", "Provide an NPM authToken for private packages.").option("-p, --proxy <http_proxy>", "Enable http proxy to connect to the registry").option('-x, --extra-args "<extra_args>"', "Extra arguments to pass through to NPM or Yarn").usage("<package>[@<version>], default version is 'latest'").parse(process.argv);
...
if (program.args.length > 1) {
  console.log(C.errorText + " Too many arguments. Please specify ONE package at a time to install with peerDeps. Alternatively, pass extra arguments with --extra-args \"<extra_args>\".");
  // An exit code of "9" indicates an invalid argument
  // See https://nodejs.org/api/process.html#process_exit_codes
  process.exit(9);
}
